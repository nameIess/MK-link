...
func PlanLink(target, destination string, linkType Type) (Plan, error) {
	...
	if err != nil {
		return Plan{}, fmt.Errorf("resolve destination path: %w", err)
	}
	...
	if err != nil {
		return Plan{}, fmt.Errorf("inspect destination folder: %w", err)
	}
	if !parentInfo.IsDir() {
		return Plan{}, errors.New("destination folder is not a directory")
	}
	if err := validateLinkName(filepath.Base(destination)); err != nil {
		return Plan{}, err
	}
	...
	case Hardlink:
		if isDirectory {
			return Plan{}, ErrTargetTypeMismatch
		}
		same, err := sameVolume(target, destination)
		if err != nil {
			return Plan{}, fmt.Errorf("compare volumes: %w", err)
		}
		if !same {
			return Plan{}, ErrDifferentVolume
		}
	case Junction:
		if !isDirectory {
			return Plan{}, ErrTargetTypeMismatch
		}
		if !isLocalVolumePath(target) || !isLocalVolumePath(destination) {
			return Plan{}, ErrNetworkJunction
		}
	default:
		return Plan{}, errors.New("unknown link type")
	}

	if isDirectory && pathInside(target, parent) {
		return Plan{}, ErrNestedDestination
	}
...
func Create(p Plan) error {
	...
	switch p.Type {
	case Symbolic:
		isDirectory, err := isDir(p.Target)
		if err != nil {
			return err
		}
...
func validateLinkName(name string) error {
	...
	for _, reserved := range []string{
		...
		if baseUpper == reserved || strings.HasPrefix(baseUpper, reserved+".") {
			return ErrInvalidLinkName
		}
	}
	return nil
...
func sameVolume(a, b string) (bool, error) {
	left := strings.ToUpper(filepath.VolumeName(a))
	right := strings.ToUpper(filepath.VolumeName(b))
	if left == "" || right == "" {
...
func createSymbolic(target, destination string, directory bool) error {
	...
	flags := uint32(symbolicLinkFlagAllowUnprivileged)
	if directory {
		flags |= symbolicLinkFlagDirectory
	}

	r1, _, callErr := createSymbolicLinkW.Call(
...
func createHardlink(target, destination string) error {
	...
	r1, _, callErr := createHardLinkW.Call(
		uintptr(unsafe.Pointer(destinationPtr)),
		uintptr(unsafe.Pointer(targetPtr)),
		0,
	)
...
func createJunction(target, destination string) error {
	...
	if err != nil {
		return fmt.Errorf("encode junction display name: %w", err)
	}

	targetUTF16 = targetUTF16[:len(targetUTF16)-1]
	printUTF16 = printUTF16[:len(printUTF16)-1]
	subBytes := len(targetUTF16) * 2
	printBytes := len(printUTF16) * 2
	dataLen := 8 + subBytes + printBytes
	if dataLen > 0xFFFF {
		return errors.New("junction target path is too long")
	}

	buffer := make([]byte, 16+subBytes+printBytes)
	putUint32LE(buffer[0:4], ioReparseTagMountPoint)
	putUint16LE(buffer[4:6], uint16(dataLen))
	putUint16LE(buffer[6:8], 0)
	putUint16LE(buffer[8:10], 0)
	putUint16LE(buffer[10:12], uint16(subBytes))
	putUint16LE(buffer[12:14], uint16(subBytes))
	putUint16LE(buffer[14:16], uint16(printBytes))
	appendUTF16LE(buffer[16:], targetUTF16)
	appendUTF16LE(buffer[16+subBytes:], printUTF16)
	...
	handle, err := openReparseDirectory(destination)
	if err != nil {
		return err
	}
	...
	var returned uint32
	r1, _, callErr := deviceIoControl.Call(
		uintptr(handle),
		uintptr(fsctlSetReparsePoint),
	...
	if r1 == 0 {
		return fmt.Errorf("set junction reparse point: %w", callErr)
	}
...
func openReparseDirectory(path string) (windows.Handle, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("encode junction path: %w", err)
	}
	...
	if err != nil {
		return 0, fmt.Errorf("open junction directory: %w", err)
	}
	return handle, nil
}
...
func putUint32LE(dst []byte, value uint32) {
	putUint16LE(dst[:2], uint16(value))
	putUint16LE(dst[2:4], uint16(value>>16))
}

func appendUTF16LE(dst []byte, value []uint16) {
	for i, char := range value {
		putUint16LE(dst[i*2:], char)
	}
}
